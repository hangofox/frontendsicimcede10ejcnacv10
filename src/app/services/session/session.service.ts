...
export class SessionService {
  ...
  //VEZ POR SESIÓN (VER loadPrivileges) PARA QUE EL MENÚ Y LOS BOTONES DE CADA CRUD PUEDAN CONSULTARLA DE FORMA
  //SÍNCRONA (hasAccess/getFunctionalityLabel) SIN NECESIDAD DE LLAMADAS ADICIONALES AL BACKEND:
  ...
  loadPrivileges(): Observable<PrivilegyRestriccAccesosUsuariosI[]> {
    return this.privilegyRestriccAccesosUsuariosService.findAllPrivilegesAndRestrictionsAccessUsers(
      undefined, undefined, undefined, undefined, undefined, undefined, this.getUserId()
    ).pipe(
  ...
  hasAccess(nombreRol: string): boolean {
  ...
  getFunctionalityLabel(nombreRol: string): string {
  ...
  setFunctionalityAndRoleContext(nombreFuncionalidad: string, nombreRol: string): void {
