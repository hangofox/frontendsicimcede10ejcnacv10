...
export class ListadoInfraestructurasArrendadasComponent implements OnInit {
  ...
  readonly form = this.fb.group({ keyword: [''], cantidad: ['10'] });
  ...
  ngOnInit(): void { this.catalogos.proveedores().subscribe(x => { this.proveedores=x; this.cdr.markForCheck(); }); this.catalogos.tiposEstructura().subscribe(x => { this.tipos=x; this.cdr.markForCheck(); }); this.unidadesService.findAllMilitaryUnits(undefined,undefined,'nombreUnidadMilitar','ASC').subscribe(x => { this.unidades=x; this.cdr.markForCheck(); }); this.listar(); }
  listar(): void { const k=this.form.value.keyword?.trim()||undefined; this.service.findAllInfraestructurasArrendadasPag(this.paginaActual,this.cantidad,undefined,k,'idInfraestructuraArrendada','ASC').subscribe(x=>{this.registros=x;this.cdr.markForCheck();}); this.service.findCountTotalRegisters(undefined,k).subscribe(x=>{this.totalRegistros=x;this.cdr.markForCheck();}); this.service.findAllInfraestructurasArrendadas(undefined,k,'idInfraestructuraArrendada','ASC').subscribe(registros=>{this.estadosUsoContadores=this.estadosUso.map(estado=>({...estado,valor:registros.filter(registro=>this.normalizarEstado(registro.estadoUsoInfraestructuraArrendada)===estado.etiqueta).length}));this.cdr.markForCheck();}); }
  ...
  abrir(m:'guardar'|'modificar'|'eliminar',x:InfraestructurasArrendadasI|null=null):void{this.spinner.mostrarAntesDeAbrir(()=>{this.modo=m;this.seleccionado=x;this.modalEdicion=true;this.cdr.markForCheck();});} ver(x:InfraestructurasArrendadasI):void{this.seleccionado=x;this.modalVista=true;} cerrar():void{this.modalEdicion=false;this.modalVista=false;this.seleccionado=null;}
  guardar(x:InfraestructurasArrendadasI):void{const modificando=!!x.idInfraestructuraArrendada;(modificando?this.service.update(x):this.service.add(x)).subscribe({next:r=>{this.mostrarMensaje('exito',r.mensaje||(modificando?'Infraestructura arrendada modificada correctamente.':'Infraestructura arrendada creada correctamente.'));this.cerrar();this.listar();},error:e=>this.mostrarMensaje('error',e.error?.mensaje||(modificando?'Error al modificar la infraestructura arrendada.':'Error al crear la infraestructura arrendada.'))});} eliminar(id:number):void{this.service.delete(id).subscribe({next:r=>{this.mostrarMensaje('exito',r.mensaje||'Infraestructura arrendada eliminada correctamente.');this.cerrar();this.listar();},error:e=>this.mostrarMensaje('error',e.error?.mensaje||'Error al eliminar la infraestructura arrendada.')});}
