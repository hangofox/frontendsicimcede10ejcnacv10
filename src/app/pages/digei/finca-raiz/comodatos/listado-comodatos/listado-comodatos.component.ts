...
export class ListadoComodatosComponent implements OnInit {
  ...
  nombreProveedor(p:HistorialProveedoresProductosServiciosI):string{return `${p.nombresProvProdOServ||''} ${p.primerApellidoProvProdOServ||''}`.trim()||p.numRegHistorialProveedorProductoOServicio;} private mostrar(m:string):void{this.mensaje=m;this.cdr.markForCheck();setTimeout(()=>{this.mensaje='';this.cdr.markForCheck();},4000);}
}
