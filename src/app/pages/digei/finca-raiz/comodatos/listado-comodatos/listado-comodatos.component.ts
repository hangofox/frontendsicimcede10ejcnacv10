...
export class ListadoComodatosComponent implements OnInit {
  ...
  ngOnInit(): void { this.terrenosService.findAllLands(undefined, undefined, undefined, 'denominacionTerreno', 'ASC').subscribe(x => { this.terrenos=x; this.cdr.markForCheck(); }); this.catalogos.getProviders().subscribe(x => {this.proveedores=x; this.cdr.markForCheck();}); this.listar(); }
  listar(): void {
    const v = this.form.getRawValue();
    const keyword = v.keyword?.trim() || undefined;
    const idTerreno = v.terreno ? Number(v.terreno) : undefined;
    this.service.findAllLandLoansPag(this.paginaActual, this.cantidad, undefined, keyword, idTerreno, 'idComodatoTerreno', 'ASC').subscribe({ next: registros => { this.registros = registros; this.cdr.markForCheck(); }, error: () => this.mostrar('No fue posible listar los comodatos.') });
  ...
  abrir(m:'guardar'|'modificar'|'eliminar',x:ComodatosI|null=null):void{this.spinner.showBeforeOpening(()=>{this.modo=m;this.seleccionado=x;this.modalEdicion=true;this.cdr.markForCheck();});} ver(x:ComodatosI):void{this.spinner.showBeforeOpening(()=>{this.seleccionado=x;this.modalVista=true;this.cdr.markForCheck();});} cerrar():void{this.modalEdicion=false;this.modalVista=false;this.seleccionado=null;}
  guardar(x:ComodatosI):void{const modificando=!!x.idComodatoTerreno;(modificando?this.service.updateLandLoan(x):this.service.addLandLoan(x)).subscribe({next:r=>{this.mostrar(r.mensaje||(modificando?'Comodato modificado correctamente.':'Comodato creado correctamente.'));this.cerrar();this.listar();},error:()=>this.mostrar(modificando?'No fue posible modificar el comodato.':'No fue posible crear el comodato.')});} eliminar(id:number):void{this.service.deleteLandLoan(id).subscribe({next:r=>{this.mostrar(r.mensaje||'Comodato eliminado correctamente.');this.cerrar();this.listar();},error:()=>this.mostrar('No fue posible eliminar el comodato.')});}
