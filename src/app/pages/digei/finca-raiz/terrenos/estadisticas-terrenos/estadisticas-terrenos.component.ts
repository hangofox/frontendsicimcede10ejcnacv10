...
@Component({
  selector: 'app-estadisticas-terrenos',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, SemaforoContadoresComponent],
  templateUrl: './estadisticas-terrenos.component.html',
  styleUrl: './estadisticas-terrenos.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class EstadisticasTerrenosComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    this.terrenosService.findAllTerrenos(undefined, keyword, unidad, 'idTerreno', 'ASC').subscribe({
      next: registros => {
        ...
        this.estadosContadores = this.estados.map(estado => ({
          ...
          tono: estado.tono,
          valor: registros.filter(registro => this.normalizar(registro.estadoTerrenoDTO?.nombreEstadoTerreno) === estado.etiqueta).length
        }));
        this.cargando = false;
      ...
      error: error => {
        console.error('ERROR AL CARGAR ESTADISTICAS DE TERRENOS:', error);
        this.cargando = false;
  ...
  limpiarFiltros(): void { this.filtrosForm.reset({ keyword: '', unidad: '' }); this.cargarEstadisticas(); }
  ...
  private dibujarGraficas(): void {
    ...
    this.graficas.push(new Chart(this.graficaTortaRef.nativeElement, {
      ...
      options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 35, right: 105, bottom: 15, left: 105 } }, plugins: { legend: { position: 'bottom' }, title: { display: true, text: 'Distribución por estado del terreno', font: { size: 15, weight: 'bold' } } } }
    }));
    this.graficas.push(new Chart(this.graficaBarrasRef.nativeElement, {
      type: 'bar', data: { labels: etiquetas, datasets: [{ label: 'Terrenos', data: valores, backgroundColor: colores, borderRadius: 6 }] },
      plugins: [valoresSobreBarrasPlugin],
      options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 20 } }, plugins: { legend: { display: false }, title: { display: true, text: 'Cantidad por estado del terreno', font: { size: 15, weight: 'bold' } } }, scales: { y: { beginAtZero: true, grace: '15%', ticks: { precision: 0 } } } }
    }));
  }
